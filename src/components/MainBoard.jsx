            ...
            return (
              <div key={index} className="flex flex-col space-y-4 mt-[6rem]">
                ...
                {tasks.map((task, index) => {
                  ...
                  return (
                    <div onClick={() => findTask(title)} key={index} className='flex cursor-pointer flex-col  task-board shadow-lg rounded-lg py-6 px-3 w-72'>
                      <div className='max-w-[15rem] space-y-2'>
                        <h4 className='text-[15px] font-bold'>{title}</h4>
                        <p className='text-[12px] text-grey font-semibold'>{completed} of {subtasks.length} subtasks</p>
                      </div>
