      ...
      <div className={`${showAddNewTaskModal ? 'main-modal show-modal' : 'main-modal'}`} onClick={closeAddNewTaskModal} >
        <form className='flex flex-col form-control modal-content min-w-[300px] w-[85%] max-w-[500px] p-6 rounded-lg space-y-5' onSubmit={onSubmitHandler} onClick={(e) => e.stopPropagation()}>
          ...
          <div className='flex flex-col space-y-2'>
            ...
            <div className={`flex items-center space-x-2 ${inputErrors.subtasksError && 'error-subtask'}`}>
              <input id='subtasks' type="text" name="subtasks" onKeyUp={handleErrorChange}  onBlur={handleErrorChange} onChange={handleAddedTaskChange} value={AddedTasks.subtasks} placeholder="e.g Take cofee break" className={`input placeholder:text-grey placeholder:font-semibold bg-inherit ${inputErrors.subtasksError ? 'error-border' : 'border-input' } text-[13px]  w-full`} />
